...
const cleanupRawXTB = rawStringFile => {
  ...
  allLines.forEach((line, i) => {
    line = line.replace(/<ph/g, `${escapeLesserThan}ph`);
    line = line.replace(/<ex>/g, `${escapeLesserThan}ex${escapeGreaterThan}`)
    line = line.replace(/<\/ex>/g, `${escapeLesserThan}/ex${escapeGreaterThan}`)
    line = line.replace(/<\/ph>/g, `${escapeLesserThan}/ph${escapeGreaterThan}`)
    allLines[i] = line;
  });
...
const getNewKeyName = node => {
  ...
  if (isAutogenerated) {
    return generateKeyByPath(node.source);
  } else {
...
const setKeysInMainCopies = keysToBeUpdated => {
...
const setKeysInOtherCopies = (keysToBeUpdated, filePath) => {
  ...
  keysToBeUpdated.forEach(key => {
    ...
    const wasNumeric = isKeyNumeric(key.old);
    if (wasNumeric) {
      ...
      const replaceGreaterThanRegExp = new RegExp(escapeGreaterThan, 'g');
      const replaceLesserThanRegExp = new RegExp(escapeLesserThan, 'g');
...
const main = () => {
  ...
  addMissingKeys(false); // assuming `yarn i18n` does not generate new messages
