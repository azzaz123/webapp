...
export class UserVerificationsService {
  ...
  public verifyPhone(phone: string, code: string): Observable<VERIFICATION_STATUS> {
    return this.userVerificationsHttpService.sendVerifyPhone(phone, code).pipe(map(mapPhoneVerificationApiToVerificationStatus));
  }
}
