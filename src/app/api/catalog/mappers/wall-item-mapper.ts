...
function mapWallItemToItemCard(item: WallItem, favoriteIds: string[]): ItemCard {
  const { id, category_id, title, description, price, images = [], type_attributes = {}, slug, reserved, bump, type } = item;

  return {
    id,
    title,
    categoryId: category_id,
    description: formatDescription(type, description, type_attributes),
