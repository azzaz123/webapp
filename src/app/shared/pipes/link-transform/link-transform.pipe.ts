...
export class LinkTransformPipe implements PipeTransform {
  ...
  public static readonly LINK_REG_EXP = /(https?:\/\/(?:www\.|(?!www))[\w\d][\w\d-]+[\w\d]\.[^\s]{2,}|www\.[\w\d][\w\d-]+[\w\d]\.[^\s]{2,}|https?:\/\/(?:www\.|(?!www))[\w\d]+\.[^\s]{2,}|www\.[\w\d]+\.[^\s]{2,})|((com\.)|(es\.)|(co\.uk\.)|(uk\.)|(fr\.)|(com\.br\.)|(pro\.)|(prod\.)|(test\.)|(beta\.)|(alfa\.)|(us\.)|(corp\.))?wallapop\.com[^\s]{0,}|fleapster\.com/g;
  // tslint:disable-next-line:max-line-length
