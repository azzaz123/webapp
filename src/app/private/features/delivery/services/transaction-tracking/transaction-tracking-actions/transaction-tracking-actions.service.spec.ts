...
describe('TransactionTrackingActionsService', () => {
  ...
  const MOCK_CARRIER_TRACKING_WEBVIEW_ACTION = MOCK_TRANSACTION_TRACKING.statusInfo[0].action;
  ...
  describe('when we handle a transaction tracking action...', () => {
    describe('and the action is a deeplink', () => {
      it('should open the mapped url in a new page', () => {
        const deeplinkURLMapped = mapDeeplinkToCustomerHelp(APP_LOCALE_MOCK, MOCK_DEEP_LINK_ACTION.payload.linkUrl);
        spyOn(window, 'open');
    ...
    describe('and the action is carrier tracking webview', () => {
      it('should open the tracking url in a new page', () => {
        ...
        expect(window.open).toHaveBeenCalledTimes(1);
        expect(window.open).toHaveBeenCalledWith(MOCK_CARRIER_TRACKING_WEBVIEW_ACTION.payload.linkUrl, '_blank');
      });
    });
  });
});
